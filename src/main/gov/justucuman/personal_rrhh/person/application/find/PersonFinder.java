...
public final class PersonFinder {
    ...
    public PersonResponse find(String id){
        return repository.search(id).map(person -> new PersonResponse(
                person.getId().value(),
                person.getDni().value(),
                ...
                person.getAddress().localityId().value(),
                person.getPhone().value()
                )).orElseThrow(() -> new PersonNotFoundException("No se encontro la persona: " + id));
    }
}
